...
def build_geometry(params):
    ...
    geom['chamber'] = {
        'x': x, 'w': params['chamber_length_mm'], 'h': params['barrel_id_mm']*1.6}
    x += geom['chamber']['w']
    geom['flange'] = {'x': x, 'w': 20.0, 'h': geom['chamber']['h']}
    x += geom['flange']['w']
    geom['barrel'] = {'x': x, 'w': params['barrel_length_mm'],
                      'id': params['barrel_id_mm']}
    x += geom['barrel']['w']
    geom['sabot'] = {'x': x, 'w': 60.0, 'od': params['sabot_od_mm']}
    geom['overall_length'] = x + geom['sabot']['w']
    # frame rails
    geom['frame'] = {'y_offset': -30.0, 'rail_w': 25.0,
                     'rail_gap': params['frame_width_mm']}
    return geom
    ...
    if svgwrite:
        ...
        def rect(x, y, w, h, stroke='black'):
            dwg.add(dwg.rect(insert=(f'{x}mm', f'{y}mm'), size=(
                f'{w}mm', f'{h}mm'), fill='none', stroke=stroke))

        rect(geom['co2_cartridge']['x'], y_center-10,
             geom['co2_cartridge']['w'], geom['co2_cartridge']['h'])
        rect(geom['chamber']['x'], y_center-geom['chamber']
             ['h']/2, geom['chamber']['w'], geom['chamber']['h'])
        rect(geom['flange']['x'], y_center-geom['flange']
             ['h']/2, geom['flange']['w'], geom['flange']['h'])
        ...
        rect(bar_x + (geom['barrel']['w']-bore_w)/2,
             y_center-bore_w/2, bore_w, bore_w, stroke='red')
        # sabot
        rect(geom['sabot']['x'], y_center-20, geom['sabot']['w'], 40)

        # dimension lines (simple)
        dwg.add(dwg.line(start=(f'{geom["co2_cartridge"]["x"]}mm', f'{y_center+80}mm'), end=(
            f'{geom["barrel"]["x"]+geom["barrel"]["w"]}mm', f'{y_center+80}mm'), stroke='black'))
        dwg.add(dwg.text(f"Overall {geom['overall_length']:.0f} mm", insert=(
            f'{10}mm', f'{y_center+95}mm'), fill='black'))
        dwg.save()
    else:
        ...
        with open(out_svg, 'w') as f:
            ...
            def r(x, y, w, h, stroke='black'):
                f.write(
                    f'<rect x="{x}" y="{y}" width="{w}" height="{h}" fill="none" stroke="{stroke}"/>\n')
            r(geom['co2_cartridge']['x'], y_center-10,
              geom['co2_cartridge']['w'], geom['co2_cartridge']['h'])
            r(geom['chamber']['x'], y_center-geom['chamber']
              ['h']/2, geom['chamber']['w'], geom['chamber']['h'])
            r(geom['flange']['x'], y_center-geom['flange']
              ['h']/2, geom['flange']['w'], geom['flange']['h'])
            bar_x = geom['barrel']['x']
            bar_h = geom['barrel']['id']*1.6
            r(bar_x, y_center-bar_h/2, geom['barrel']['w'], bar_h)
            bore_w = geom['barrel']['id']
            r(bar_x + (geom['barrel']['w']-bore_w) /
              2, y_center-bore_w/2, bore_w, bore_w)
            r(geom['sabot']['x'], y_center-20, geom['sabot']['w'], 40)
            f.write(
                f'<line x1="{geom["co2_cartridge"]["x"]}" y1="{y_center+80}" x2="{geom["barrel"]["x"]+geom["barrel"]["w"]}" y2="{y_center+80}" stroke="black"/>\n')
            f.write(
                f'<text x="10" y="{y_center+95}">Overall {geom["overall_length"]:.0f} mm</text>\n')
            f.write('</svg>')
...
def write_png(geom, params, out_png):
    ...
    draw_rect(geom['co2_cartridge']['x'], y_center-10,
              geom['co2_cartridge']['w'], geom['co2_cartridge']['h'])
    draw_rect(geom['chamber']['x'], y_center-geom['chamber']
              ['h']/2, geom['chamber']['w'], geom['chamber']['h'])
    draw_rect(geom['flange']['x'], y_center-geom['flange']
              ['h']/2, geom['flange']['w'], geom['flange']['h'])
    bar_x = geom['barrel']['x']
    bar_h = geom['barrel']['id']*1.6
    draw_rect(bar_x, y_center-bar_h/2, geom['barrel']['w'], bar_h)
    bore_w = geom['barrel']['id']
    draw_rect(bar_x + (geom['barrel']['w']-bore_w) /
              2, y_center-bore_w/2, bore_w, bore_w)
    draw_rect(geom['sabot']['x'], y_center-20, geom['sabot']['w'], 40)
    cv2.putText(img, f'Overall {geom["overall_length"]:.0f} mm',
                (10, y_center+140), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 2)
...
def main():
    ...
    try:
        ...
        doc.add_page_break()
        doc.add_heading(
            'Engineering Drawing (automatically generated)', level=1)
        doc.add_paragraph(f'Files: {os.path.relpath(out_svg, REPO_ROOT)}, {os.path.relpath(out_png, REPO_ROOT)}' + (
            f', {os.path.relpath(out_dxf, REPO_ROOT)}' if wrote_dxf else ''))
        doc.save(master)
    except Exception:
        pass
